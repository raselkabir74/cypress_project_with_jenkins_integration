...
describe('Verify Banner type Campaign Creation, Approval and Bidding with custom URL/pixel impression tracking', () => {
  let originalBaseUrl
  let customScreenshotName

  before(() => {
    originalBaseUrl = Cypress.config('baseUrl')
    customScreenshotName = `banner_type_campaign_for_pixel_impression_tracking`
  })
  ...
  describe('Campaign creation and approve', () => {
    ...
    it('Create campaign', () => {
      cy.fixture('banner_type_campaign_with_pixel_impression_tracking.json').as('campaign')
  ...
  describe('Bid request and response validation', () => {
    it('Send first success bid requests', () => {
      cy.fixture('../bid_request/banner_type_campaign_with_pixel_impression_tracking.json').as('successBidRequest')
    ...
    it('Verify bid response', { baseUrl: null }, () => {
      ...
      cy.get('@createdCampaign').then((campaign) => {
        ...
        BID_RESPONSE_ASSERTION.verifyBidResponse({
          snapName: 'banner_type_campaign_with_pixel_impression_tracking',
          URLTracking: true,
        })
      })
    })
